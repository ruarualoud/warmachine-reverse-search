...
| 编号 | 缺陷 | 严重程度 | 对应整改 |
|---|---|---|---|
| 一 | 计划级合同版本作废了与语义变化无关的已结算证据 | 严重 | R0 + R2 |
| 二 | 几何枚举分母在当前实现下不可完成 | 严重 | Ticket 05a |
| 三 | 缺少射程预筛、角度等价折叠与支配剪枝 | 严重 | Ticket 05a |
| 四 | Engine CI 分片验证曾静默虚假通过 | 高 | R1 |
| 五 | Ticket 14 折叠能力排序错置于其依赖者之后 | 高 | 拆出 Ticket 05a |
| 六 | 最终目标缺少必要条件 Ticket 08 | 高 | Ticket 08 提升至关键路径 |
| 七 | 三处状态记录互相矛盾 | 中 | R0 |
