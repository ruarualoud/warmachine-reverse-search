...
调度器支持等待已有生产 PID 后无缝接管、单实例锁、最小剩余磁盘门、无进展失败关闭、可选循环预算，以及包含 canonical 计数和池内 chunk 进度的内建心跳。默认 `max-cycles=0`，即持续运行到 `searchFinished=true`；它不把预算停止、未解析规则或连续域债务解释成搜索闭合。
