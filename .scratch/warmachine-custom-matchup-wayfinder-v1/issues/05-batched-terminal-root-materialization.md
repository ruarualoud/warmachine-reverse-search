...
优化任务 `OPT-05` 已直接列于本工单的“开发补充”中；[原理审计](../../../docs/research/ticket-search-principles-audit-20260905.md) 仅保留依据与索引。`Blocked by` 表示最终完成依赖，历史验收范围不追溯改变。
...
## 开发补充：OPT-05（2026-09-05）

以下为历史验收之外的维护/扩展待办，当前全部待实现/待验收；保留原完成状态，不要求重跑全部历史搜索。由相关开放工单集成时执行，不把它们新增成历史关闭门。

- [ ] 终局物化按需兑现几何见证，复用带状态域/来源的碰撞、资源和时序冲突；一个点失败不得排除整个组。
- [ ] 调度同时记录各需求组的已处理量、未解析和最老等待项，避免容易的根耗尽全部预算。
- [ ] 用同一有限任务对照全量与按需物化的结果集合、严格拒绝及遗漏账；同时报告生成/证明/Host/存储成本。
