...
进度（2026-09-15）：已完成动态关联分母、前四批手工精确证据及五类确定性原子关系批次。旧 `67` 节点手写图降为历史诊断；当前报告 `af5592c6...d0819` 从 `149` 条语义、`1140` 个原子、`1354` 个 hook/consumer、显式关联声明和状态读写关系自动派生 `65` 条语义关系、`450` 条读写关系、`1523` 条原子关联及 `49` 个 hook 并发组。全部 `9` 个语义到原语目录缺口已闭合。手工 binding 关闭 `16` 条义务，单关系字面夹具账本关闭 `705` 条，多关系精确键账本现关闭 `91` 条，受审动态夹具账本关闭 `108` 条，两组受审授予协议逐义务关闭 `128` 条，受审多夹具账本关闭 Cryx 的 `6` 条关系及 Nyrro Vermin Feed 的 boxed-memory 关系。Nyrro 支援验证器已改用原子真实 interaction key，并只让已有完整行为见证的 Crimson Ballet 新增一条精确绑定；Cryx Thrall Crafting 的既有严格场景现明确绑定其实际证明的原子支付/放置、当前模型身份/入场和动态全局上限三条关系。Frosted Earth 的 stationary 分支、Echolocation、Expendable Pawn 及 Vermin Feed 的激活结束移动关系仍保持未决。交互门现含十类 killed mutation，Authority `a7be8eaf...d5b7b` 继续失败关闭。剩余 `57/444/482` 关系及 `38` 个多规则 hook 组保持未决；未经逐关系审定的多夹具、自定义夹具及未组合交互必须另给精确证据。所有结构证明不扩展 Cryx/Nyrro加共享核心的跨运行时接口势力范围。23.9 父纵切和 Ticket 23 不晋级。
