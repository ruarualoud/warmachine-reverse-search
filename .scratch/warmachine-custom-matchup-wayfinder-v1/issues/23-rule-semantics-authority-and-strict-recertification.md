...
- [ ] 23.10覆盖桌面快照 -> Host状态 -> 搜索候选/执行 -> 报告回放的同源校验。对资源、已用能力、持续状态、待处理选择或合法路径做单项改动时，搜索读取到的选项和Host实际接受结果必须一致；合法性仍只由Engine裁决。
- [ ] 与20/13绑定查询所需规则闭包：既包括推荐路线使用的能力，也包括双方可用/可授予的反制及候选发现依赖。缺一项时保留规则未知或研究候选，不能仅因当前路线可执行就认证局面最佳建议。
- [ ] 分别验收非法人工位置、缺必需历史、过期Host收据和真实终局后的动作；当前状态条件分析无需证明部署历史，但不能因此省略未来规则所需信息。规则认证不替代21的对抗值/最优性证明，也不授权自动落子。
...
进度（2026-09-15）：已完成动态关联分母、前四批手工精确证据及五类确定性原子关系批次。旧 `67` 节点手写图降为历史诊断；当前报告 `313b0786...b1ae` 从 `149` 条语义、`1140` 个原子、`1354` 个 hook/consumer、显式关联声明和状态读写关系自动派生 `65` 条语义关系、`450` 条读写关系、`1523` 条原子关联及 `49` 个 hook 并发组。全部 `9` 个语义到原语目录缺口已闭合。手工 binding 关闭 `16` 条义务，单关系字面夹具账本关闭 `705` 条，多关系精确键账本现关闭 `108` 条，受审动态夹具账本关闭 `108` 条，两组受审授予协议逐义务关闭 `128` 条，受审多夹具账本关闭 Cryx 的 `6` 条关系及 Nyrro Vermin Feed 的 boxed-memory 关系。Nyrro 支援验证器已改用原子真实 interaction key，并只让已有完整行为见证的 Crimson Ballet 新增一条精确绑定；Cryx Thrall Crafting、Grim Returns、Wrath of Lyliss、Reduce/Reuse/Recycle、Graveyard Shift 与 Synergy 的既有严格场景现明确绑定其实际执行的关系。Frosted Earth 的 stationary 分支、Echolocation、Expendable Pawn 及 Vermin Feed 的激活结束移动关系仍保持未决。交互门现含十类 killed mutation，Authority `e7f3b8a7...47ea51` 继续失败关闭。剩余 `57/444/465` 关系及 `38` 个多规则 hook 组保持未决；未经逐关系审定的多夹具、自定义夹具及未组合交互必须另给精确证据。所有结构证明不扩展 Cryx/Nyrro加共享核心的跨运行时接口势力范围。23.9 父纵切和 Ticket 23 不晋级；为尽快产出第一份可信搜索结果，下一关键路径改为固定 Cryx/Nyrro 任务的依赖闭包和范围内认证，任何实际遇到的未闭合依赖仍严格拒绝，全局债务不进入该有限结论。
...
| 23.9 | 进行中，动态分母与五类确定性账本已存在 | 当前自动图含65条语义关系、450条状态关系、1523条原子关系和49个并发组；已精确绑定1052条义务，10/10图级mutation被杀死 | 为剩余57/444/485/38项补逐义务组合证据，不以共享夹具或名称推断覆盖 | 未受审多夹具、自定义夹具和真实组合交互仍未闭合 |
