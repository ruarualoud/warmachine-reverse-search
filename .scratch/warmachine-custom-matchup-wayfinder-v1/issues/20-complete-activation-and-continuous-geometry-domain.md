...
- [x] 20a.2a：把同一顺序窗口协议扩展到单移动模型的多个Free Strike；每个放弃/使用后重新查询存活攻击者和当前攻击资格，首个致死时后续机会消失。该纵切仅用于显式`legacy_free_strike`规则模式，不改变当前MKIV离战规则。
- [x] 20a.2b：主攻击的伤害转移改为伤害确定后、应用前的防守方Host窗口；拒绝与选择接收者均重放同一攻击/伤害骰，选择接收者后只补接收者受伤位置随机量。
- [ ] 20a.2c：Unit正常移动，以及Graveyard Shift/Thrall Crafting无危险地形竞争的整批放置回应子域已关闭；显式legacy Free Strike已在底盘离开近战范围的精确路径点中断，并按不同路径点先后、同点非主动方排序结算。继续扩展危险地形与敌方反应的主动方/非主动方同触发排序、Free Strike命中后push/pull改写剩余路线及同一敌人的重复离战、中断/同时/次级伤害转移及其它复合回应。只有依赖证明允许时，才能一次性展开独立选择的笛卡尔积。
...
- 2026-09-17 完整激活生产恢复边界：首次 `4096` 工作 rolling 进程已严格写出 `166` 个四工作分片且无 strict 失败，但父进程把全部约 `21 MB` 的解析后分片常驻内存，在 canonical 已发布 `576` 工作后触发 Node 4 GB heap OOM。后续生产改为每子进程最多 `256` 工作的连续精确 epoch；每个 epoch 仍使用同一 base 的不相交工作、原有合并审计和原子 canonical 提交，成功后退出释放堆并压缩封存结果。OOM 后未进入最后收据的 `88` 工作从 canonical 重算，不手工接到账本；这是执行存储修复，不改变动作、概率、对手回应或搜索值。
...
- 2026-09-15 Nyrro Pursuit 参数域跨层纵切：Engine 在真实当前 Ashmael 状态中为被选 `jack-in` 签发开放凸域普遍执行合同，Search 验证动作/施法者/模型/谓词/见证/定理绑定并保留 Ashmael 与目标的不可达碰撞证明。该战机的全部百分之一英寸终点均有一条严格直线路径；Ashmael 本身仍因未闭合规则表面而是未认证的另一合格选择，所以整个反应的 `destinationParameterLegalityComplete=false`，路径类别、转移稳定和连续回应域也继续未完成。既有对手回应门只覆盖 Cryx/Nyrro 与通用最小接口，一次通过，未运行其它势力矩阵。
...
- 2026-09-15 主攻击伤害转移纵切：Engine在严格伤害已确定、尚未应用时打开`damageTransferWindow`，窗口记录原攻击和伤害骰、当前伤害量、合法接收者及防守方所有权；拒绝重放原动作，转移重放Host原先枚举的接收者变体，仅让接收者位置/Tough随机量覆盖原目标局部结果。Search当前窗口能遍历两项并取得真实后继。12点伤害实测中，拒绝使5血warlock死亡；转移花1 Fury，warlock保持5血，27血战兽降到15。旧精确Chance/min变体路径保留；中断攻击、同时/次级伤害、完整Essence证明和后响应Chance分母仍开放。
- 2026-09-15 Unit正常移动同时落位回应纵切：Engine保持选中模型先advance、其余模型全部同时place的状态顺序，并把落位反应身份扩展为“规则+反应者+目标+事件”；同一Defensive Strike模型面对两个落位目标时不再被反应者键折叠。Search通过现有当前窗口遍历真实Host后继：初窗同时公开两个目标，放弃`placed-one`后只剩`placed-two`，随后使用反击只击杀`placed-two`并关闭窗口。旧静态回应投影因同一反应者映射多个目标而正确报告碰撞，不宣称完整；动态Host窗口才是执行权威。该证据只覆盖Unit正常移动，特殊整队放置、精确Free Strike中断/续走、复合伤害转移和完整Chance仍开放。
- 2026-09-15 特殊整批放置回应纵切：Graveyard Shift与Thrall Crafting验证不再按反应者折叠目标，所有新模型先进入同一原子状态，再由Host逐项重查非主动方回应。Graveyard Shift实测同一Defensive Strike面对多个新Grunt时可先放弃一名、再攻击另一名；Thrall Crafting实测三个目标逐个放弃后只支付一次三枚尸体并生成完整三只Crawler。同一模型的Defensive Strike使用记录在随后放弃Admonition后仍保留，证明多规则历史没有覆盖。若同一落位批次同时有危险地形伤害和敌方可选反应，当前以显式未解析原因失败关闭，等待主动方/非主动方同触发排序协议；这部分与精确Free Strike续走、复合伤害转移和完整Chance一起留在20a.2c。
- 2026-09-15 自由打击精确路径中断纵切：显式`legacy_free_strike`模式沿实际声明路径计算底盘离开敌方近战范围的精确中断点，不再以起终点差集替代时序；路径从范围外进入后再离开同样可见。不同中断点按累计路径距离先后结算，同一点才接受非主动方给出的次序；攻击前移动者处在中断点，后续路线随窗口保存，前一攻击使其不能继续时不再到达终点。现有中断、Drag与Skewer门通过。命中后的push/pull续走以及同一敌人的重复离战仍严格拒绝并留在下一纵切；该证据不改变MKIV离战规则，也不刷新全局收据。
