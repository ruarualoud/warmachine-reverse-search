...
优化任务 `OPT-03` 已直接列于本工单的“开发补充”中；[原理审计](../../../docs/research/ticket-search-principles-audit-20260905.md) 仅保留依据与索引。`Blocked by` 表示最终完成依赖，历史验收范围不追溯改变。
...
## 开发补充：OPT-03（2026-09-05）

以下为历史验收之外的维护/扩展待办，当前全部待实现/待验收；保留原完成状态，不要求重跑全部历史搜索。由相关开放工单集成时执行，不把它们新增成历史关闭门。

- [ ] 复用Force Builder；背包分支保留点数、FA、附件、逐副本配装和控制群约束，目标能力只能提供经证明的必要条件/乐观界。
- [ ] 在一个能完整列出的合法军表小域上对照生成集合；构筑评分仅排序，截断保留候选池外债务。
- [ ] 试做有限军表池的最佳回应扩展，记录未纳入军表及回应求解缺口；冻结先后选表或同时选表的任务语义，不将池内最优外推全种族。
...
- [ ] 为10的新查询入口复用军表/配装身份与任务版本绑定，区分势力构筑域、精确当前局面起点和指定目标谓词；局面模式不得静默重置模型损失、资源或回合历史。此为原构筑验收外的接口扩展。
- [ ] 军表对抗结果同时记录地图/先手处理、先后或同时选表语义、候选池遗漏及值界；没有优势纯军表时保留多个候选或受限池混合解，不用逐表最大值冒充对抗均衡。
