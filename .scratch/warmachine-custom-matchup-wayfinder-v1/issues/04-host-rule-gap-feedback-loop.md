...
优化任务 `OPT-04` 已直接列于本工单的“开发补充”中；[原理审计](../../../docs/research/ticket-search-principles-audit-20260905.md) 仅保留依据与索引。`Blocked by` 表示最终完成依赖，历史验收范围不追溯改变。
...
## 开发补充：OPT-04（2026-09-05）

以下为历史验收之外的维护/扩展待办，当前全部待实现/待验收；保留原完成状态，不要求重跑全部历史搜索。由相关开放工单集成时执行，不把它们新增成历史关闭门。

- [ ] 按精确来源、consumer、时序窗口和Host收据归并同因失败；strict非法、输入缺失、未实现规则、预算停止分别入账。
- [ ] 失败证据包含choiceOwner、时点和后继影响；对手回应的规则缺口回Engine修复，不在Search补规则解释。
- [ ] 以同一缺陷触发多个根的场景验证归并；修复后只失效依赖子图，未知范围保守扩大，不把未知规则永久缓存为无动作。
