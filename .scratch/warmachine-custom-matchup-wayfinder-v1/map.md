...
| 变更组 | Ticket | 本次调整性质 |
|---|---|---|
| 核心开发路线 | 20、21、22 | 参数/回应域与未解析接口；多回合适应性对抗；按需策略精化与安全剪枝。20最终依赖显式补23 |
| 既有基础接口 | 08 | 复用已有min/max/Chance，补生产逐节点接口与量词回归，不重写或重开历史值代数验收 |
| 报告与产品集成 | 09、10、11、12、13、23 | 对手反制展示、任务目标模式、机制闭包、候选发现依赖、分级证据与规则决策方/时序 |
| 历史维护优化 | 01-07、14-19 | 各自记录复用、分组、分页、恢复、缓存与反例待办，不加新ticket、不重跑已验收全部路线 |

对手回应合同与12项R21验收直接见[Ticket21](issues/21-multiturn-adversarial-chance-closure.md)。表中的新增项均待开发，当前状态仍为14完成/9开放；已完成ticket中的优化项是明确的历史范围外维护，不隐瞒新增工作量。
